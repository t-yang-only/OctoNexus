...
func TestPickGroupItemByModeLowestCostBeatsPriority(t *testing.T) {
	...
	if got := pickGroupItemByMode(legacyGroup, routeDeps{cost: cost}, false); got.ID != 101 {
	...
	if got := pickGroupItemByMode(costGroup, routeDeps{cost: cost}, false); got.ID != 102 {
	...
	if got := pickGroupItemByMode(costGroup, routeDeps{cost: cost}, true); got.ID != 102 {
...
func TestPickGroupItemByModeKeepsManualPath(t *testing.T) {
	...
	if got := pickGroupItemByMode(group, routeDeps{cost: cost}, false); got.ID != 111 {
		t.Fatalf("manual picked %d, want the active item 111", got.ID)
	}
	if got := pickGroupItemByMode(group, routeDeps{cost: cost}, true); got.ID != 111 {

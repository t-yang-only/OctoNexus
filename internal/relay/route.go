...
func pickGroupItemHot(group model.Group) model.GroupItem {
	return pickGroupItemHotWithFeatures(group, SmartRoute{})
}

// pickGroupItemHotWithFeatures 是带请求特征与档位切分点的选路入口：只有智能路由模式会读它，
// 其余模式必须完全不受影响（零值时与 pickGroupItemHot 逐字一致）。
func pickGroupItemHotWithFeatures(group model.Group, smart SmartRoute) model.GroupItem {
	...
	return pickGroupItemByModeWithFeatures(group, hotRouteDeps(), RouteBalanceEnabled(), smart)
}
...
func pickGroupItemByMode(group model.Group, deps routeDeps, balanceEnabled bool) model.GroupItem {
	return pickGroupItemByModeWithFeatures(group, deps, balanceEnabled, SmartRoute{})
}
...
func pickGroupItemByModeWithFeatures(group model.Group, deps routeDeps, balanceEnabled bool, smart SmartRoute) model.GroupItem {
	switch {
	case group.Mode == model.GroupModeSmart:
		return pickGroupItemSmart(group, deps, balanceEnabled, smart)

...
// isAutoGeneratedName 判断分组名是不是「渠道名/模型名」这个自动生成形态。
//
// 与后端 op.isAutoGroupName 同一口径：含斜杠且两侧都非空。
// 只用于展示过滤，不参与任何删除决策 —— 隐藏 ≠ 取消选择。
...
function APIKeyForm({ apiKey, isPending, submitLabel, onSubmit, onClose }: APIKeyFormProps) {
    ...
    // 模型选择器的搜索词与「隐藏自动生成分组」开关。
    // 生产实测：418 个分组里 279 个是自动生成的，平铺在一个滚动区里很难找。
    ...
    // 搜索 + 隐藏自动生成分组的过滤。
    //
    // 「自动生成」按名字形态判断（含斜杠且两侧都非空，即「渠道名/模型名」）——
    // 与后端 isAutoGroupName 同一口径。这里只做展示过滤，
    // 不改动 supported_models 里已经选中的项（隐藏不等于取消选择）。
    const visibleModels = availableModels.filter((name) => {
        if (hideAuto && isAutoGeneratedName(name) && !supportedModels.has(name)) return false;
    ...
    return (
        <form onSubmit={handleSubmit} className="grid gap-2">
            ...
            <div className="grid gap-1">
                ...
                {/* 搜索框 + 已选/仅自动的过滤。
                    生产实测：418 个分组里 279 个是「渠道名/模型名」形态的自动生成分组，
                    全平铺在一个 max-h-40 的滚动区里，找目标基本靠肉眼扫 —— 这个框就是修它。 */}
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    {form.supported_models.length > 0 && (
                        <button
                            ...
                            onClick={() => updateForm({ supported_models: [] })}
                        >
                            {t('apiKey.form.clearSelection', { count: form.supported_models.length })}
                        </button>
                    )}
                    <button
                        type="button"
                    ...
                    >
                        {hideAuto ? t('apiKey.form.showAuto') : t('apiKey.form.hideAuto')}
                    </button>
                </div>
                <div className="max-h-40 overflow-auto rounded-xl p-2">
                    {visibleModels.length === 0 ? (
                        <div className="text-xs text-muted-foreground py-2 text-center">
                            {/* 三种空要分开说：一个分组都没有、被搜索挡掉、被过滤挡掉。 */}
                            {t(
                                availableModels.length === 0
                                    ? 'apiKey.form.noModels'
                                    : 'apiKey.form.noMatchingModels'
                            )}
                        </div>
                    ) : (
                ...
                {/* 显示/总数：搜索后看不出"还有多少没显示"会让人以为丢了模型。 */}
                {(modelSearch.trim() !== '' || hideAuto || visibleModels.length !== availableModels.length) && (
                    <div className="text-[11px] text-muted-foreground">
                        {t('apiKey.form.filteredCount', {

...
| T-group-001 | 分组嵌套数据模型：GroupItem 支持引用子分组，防循环+级联语义+迁移 | group | todo | - | 2026-09-13 | W0-1 终裁不过门（148）：当前树无 ChildGroupID/013 迁移/嵌套测试，113 产物灭失未重建、134 复核无落盘；按 107 拆分口径重建，A 车道认领，完成才放行 T-group-002/003 |
...
| T-acct-004 | 手动订阅+通用 HTTP 余额：无接口套餐手工录入；自定义 JSON 余额接口适配 | account | review | cursor-local | 2026-09-13 | 148 裁决：145 实质完成 custom_balance.go+6 单测但违反 146 号令提前执行（记录违规一次，下不为例）；适配核心待验收；手动订阅落库/UI 属 W2，开工前用户须拍板存储位置（渠道凭据 vs 独立表） |
